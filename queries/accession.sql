...
src AS (
  SELECT
    -- Clean version for normalization/sorting
    regexp_replace(
    ...
    -- Also clean the "original" call number for display
    regexp_replace(
      irp.call_number,
      '(\||\$)[a-zA-Z]',
      '',
...
SELECT 
	...
	TO_CHAR(ir.inventory_gmt, 'MM-DD-YYYY') AS "Inventory Date"
...
ORDER BY 
    -- Sort numerically by leading digits first
    CASE
